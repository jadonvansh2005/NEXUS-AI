...
export default function LoginPage() {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage("Authentication Successful! Redirecting to chat...");
      setTimeout(() => {
    ...
    <div className="relative min-h-screen bg-[#050505] text-white flex flex-col justify-center items-center px-6 py-12 overflow-hidden font-sans noise-bg">
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-zinc-400 text-xs font-medium uppercase tracking-wider">Access Universal personal Smart System</p>
        </div>
        ...
        {errorMessage && (
          <motion.div 
            ...
            className="flex items-center gap-2.5 p-3.5 mb-6 rounded-xl border border-rose-500/20 bg-rose-500/5 text-rose-400 text-sm font-medium"
          >
        ...
        {successMessage && (
          <motion.div 
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="flex items-center gap-2.5 p-3.5 mb-6 rounded-xl border border-emerald-500/20 bg-emerald-500/5 text-emerald-400 text-sm font-medium"
          >
        ...
        <form onSubmit={handleLoginSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              ...
              <a href="#" className="text-xs font-semibold text-[#e056fd] hover:underline">Forgot?</a>
            </div>
            <div className="relative flex items-center bg-[#111118]/80 border border-zinc-800 rounded-xl px-4 py-3.5 focus-within:border-[#e056fd]/50 transition-colors">
          ...
          <button 
            type="submit"
            disabled={loading || googleLoading}
            className="w-full flex items-center justify-center gap-2 py-4 rounded-xl font-bold text-white bg-gradient-to-r from-[#e056fd] to-[#be2edd] transition-transform hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50 disabled:scale-100 shadow-[0_0_20px_rgba(224,86,253,0.2)]"
          >
            {loading ? (
        ...
        <button 
          ...
          disabled={loading || googleLoading}
          className="w-full flex items-center justify-center py-4 rounded-xl font-bold border border-zinc-800 bg-[#111118]/80 hover:bg-[#151522]/80 hover:border-zinc-700 transition-transform hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50 disabled:scale-100"
        >
          {googleLoading ? (
        ...
        <div className="mt-8 text-center text-sm font-medium text-zinc-400">
          ...
          <Link href="/signup" className="font-bold text-[#e056fd] hover:underline">
            Sign Up
          </Link>
        </div>
      </motion.div>
